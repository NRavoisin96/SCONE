...
@TestCase
  ...
  ! Test paramethers
  integer(shortInt), parameter           :: G0 = 3, lev1_uni = 5, lev1_uniRoot = 67, lev2_uni = 3, lev2_uniRoot = 32
  real(defReal), parameter               :: E0 = 0.00034_defReal, t0 = 13.0_defReal, w0 = 0.7_defReal
  real(defReal), dimension(3), parameter :: lev1_offset = [0.3_defReal, -13.5_defReal, ONE], &
                                            lev2_offset = [-3.4_defReal, 3.0_defReal, -8.0_defReal], &
                                            r0 = [-1.3_defReal, 97.7_defReal, -9.0_defReal], u0 = [ONE, ZERO, ZERO]
...
contains
  ...
  subroutine setUp(this)
    ...
    ! Set MatIdx
    call this % p_MG % coords % setMaterialIdx(7)
    call this % p_CE % coords % setMaterialIdx(7)
    ...
    ! Level 1
    call this % p_MG % coords % setUniverseIdx(1, 1)
    call this % p_MG % coords % setUniverseRootId(0, 1)
    call this % p_MG % coords % setLocalId(1, 1)
    call this % p_MG % coords % setCellIdx(3, 1)

    call this % p_CE % coords % setUniverseIdx(1, 1)
    call this % p_CE % coords % setUniverseRootId(0, 1)
    call this % p_CE % coords % setLocalId(1, 1)
    call this % p_CE % coords % setCellIdx(3, 1)

    ! Level 2
    call this % p_MG % coords % setUniverseIdx(lev1_uni, 2)
    call this % p_MG % coords % setUniverseRootId(lev1_uniRoot, 2)
    call this % p_MG % coords % setLocalId(4, 2)
    call this % p_MG % coords % setCellIdx(2, 2)

    call this % p_CE % coords % setUniverseIdx(lev1_uni, 2)
    call this % p_CE % coords % setUniverseRootId(lev1_uniRoot, 2)
    call this % p_CE % coords % setLocalId(4, 2)
    call this % p_CE % coords % setCellIdx(2, 2)

    ! Level 3
    call this % p_MG % coords % setUniverseIdx(lev2_uni, 3)
    call this % p_MG % coords % setUniverseRootId(lev2_uniRoot, 3)
    call this % p_MG % coords % setLocalId(2, 3)
    call this % p_MG % coords % setCellIdx(8, 3)

    call this % p_CE % coords % setUniverseIdx(lev2_uni, 3)
    call this % p_CE % coords % setUniverseRootId(lev2_uniRoot, 3)
