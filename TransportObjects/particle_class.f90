...
!!<><><><><><><>><><><><><><><><><><><>><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
  ...
  pure function getUniIdx(self,n) result(idx)
    ...
    idx = self % coords % getUniverseIdx(n_loc)
  ...
  pure function getMatIdx(self) result(matIdx)
    ...
    matIdx = self % coords % getMaterialIdx()
  ...
  pure function getType(self) result(type)
    ...
    type = merge(P_NEUTRON_MG, P_NEUTRON_CE, self % isMG)
...
!!<><><><><><><>><><><><><><><><><><><>><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
  ...
  !!
  elemental subroutine setMatIdx(self, materialIdx)
    class(particle), intent(inout) :: self
...
!!<><><><><><><>><><><><><><><><><><><>><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
  ...
  subroutine display_particle(self)
    ...
    state = self
    call state % display()
    print *, 'Material: ', self % coords % getMaterialIdx()
...
!!<><><><><><><>><><><><><><><><><><><>><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
  ...
  subroutine particleState_fromParticle(LHS,RHS)
    ...
    ! Save all indexes
    LHS % matIdx   = RHS % coords % getMaterialIdx()
