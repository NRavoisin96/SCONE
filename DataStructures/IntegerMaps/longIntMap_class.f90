module longIntMap_class
  ...
  type, private :: content
    ...
    integer(longInt)  :: key = 0
    integer(shortInt) :: val
  end type

  !!
  !! Maps integers from longInt to shortInt
  !!
  ...
  !!
  type, public :: longIntMap
    private
    integer(shortInt)                        :: nextExponent
...
contains
  ...
  !!
  !! Initialise longIntMap with a desired size
  !!
  ...
  subroutine init(self, N)
    class(longIntMap), intent(inout) :: self
    integer(shortInt), intent(in)    :: N
    integer(shortInt)                :: N_bar, nextPow2
  ...
  pure function length(self) result(L)
    class(longIntMap), intent(in)  :: self
    integer(shortInt)          :: L
  ...
  subroutine add(self, key, val)
    ...
    elseif (real(self % Load + 1) / self % N > MAX_LOAD) then ! Double storage space
      call self % grow()

    end if

    ! Calculate Hash
    hash = knuthHash(key, self % nextExponent) + 1
  ...
  function get(self, key) result(val)
    ...
    character(*), parameter       :: Here = 'get (intMap_class.f90)'

    ! Give error if map is uninitialised (empty)
    if (self % Load == 0) call fatalError(Here, 'Target key: '//numToChar(key)// ' was not found. Map is empty.')
  ...
  !!   None
  !!
  elemental function begin(self) result(idx)
    class(longIntMap), intent(in) :: self
  ...
  function atVal(self, idx) result (val)
    ...
    integer(shortInt)             :: val
    character(*), parameter       :: Here = 'atVal (intMap_class.f90)'

    ! Check bounds
    if (idx < 1 .or. idx > self % N) &
    call fatalError(Here, 'Index is outside of bounds or map is uninitialised: '//numToChar(idx)//'.')
    if (self % map(idx) % status /= TAKEN) call fatalError(Here, 'Index refers to unoccupied entry: '//numToChar(idx)//'.')
  ...
  function atKey(self, idx) result(key)
    ...
    integer(shortInt), intent(in) :: idx
    integer(longInt)              :: key
    character(*), parameter       :: Here = 'atKey (intMap_class.f90)'
  ...
  subroutine atSet(self, val, idx)
    class(longIntMap), intent(inout) :: self
    integer(shortInt), intent(in)    :: val, idx
    character(*), parameter          :: Here = 'atSet (intMap_class.f90)'
  ...
  function next(self, idx) result(next_idx)
    ...
    character(*), parameter       :: Here = 'next (intMap_class.f90)'
  ...
  !!   None
  !!
  elemental function end(self) result(idx)
    class(longIntMap), intent(in) :: self
    integer(shortInt)             :: idx
  ...
  subroutine grow(self)
    class(longIntMap), intent(inout) :: self
    type(longIntMap)                 :: tempMap
    integer(shortInt)                :: i

    ! When growing rehashing is required. We will just create new map and copy all values in
    ...
    self % nextExponent = tempMap % nextExponent
    self % N = tempMap % N
